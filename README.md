# Project Sonic 2 Nick Arcade

Project Sonic 2 Nick Arcade, or Project S2 NA, is a passion project aiming to create a unique Sonic 2 with the familiarity of Sonic Retro's s1disasm, whilst also being compatible with the guides dedicated to both dissasemblies ([s1disasm](https://github.com/sonicretro/s1disasm) and [s2disasm](https://github.com/sonicretro/s2disasm)).

# Credits

- Filter — for which the groundworks for this project were based upon.
- MDTravisYT — for inspiration.
- Alex Field and RepellantMold — for their previous updates to the original Sonic 2 Nick Arcade Dissasembly.
- Hivebrain and Rika_Chou — for their earliest contributions.
- drx — for the original dissasembly.

# Check out their dissasemblies!

### Sonic Retro

https://github.com/sonicretro/s1disasm
https://github.com/sonicretro/s2disasm

### Filter

https://github.com/Totally-Not-Filter/s2-nick-arcade-disasm

### MDTravisYT

https://github.com/MDTravisYT/sonic-to-1

# Made with ♥ by [year0c](https://github.com/year0c)
